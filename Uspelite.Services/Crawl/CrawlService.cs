        ...
        public void ParseNews(int id, string userId, int portionCount = 1000)
        {
            ...
            switch ((CrawlType)id)
            {
                case CrawlType.DarikNews:
                    this.DarikNewsParser(id, crawlCount, userId, portionCount);
                    break;
            }
        }

        private void DarikNewsParser(int id, int from, string userId, int portionCount)
        {
            ...
            for (int i = from; i <= to; i++)
            {
                ...
                if (articleContent != null)
                {
                    ...
                    var picture = this.imagesService.SaveImageFromWeb(imageUrl, title, ImageFormat.Png, userId);
